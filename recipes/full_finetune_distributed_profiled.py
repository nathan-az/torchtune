...
class FullFinetuneRecipeDistributed(FTRecipeInterface):
    ...
    def _setup_model(
        ...
        self.fp8_handler = training.Float8Handler(
            ...
            dp_shard_enabled=self.parallel_dims.dp_shard_enabled,
        )
    ...
    def train(self) -> None:
        ...
        for curr_epoch in range(self.epochs_run, self.total_epochs):
            ...
            for idx, batch in enumerate(self._dataloader):
                ...
                log.info(
                    f"[rank {self.rank} step {self.fwd_step}] fwd time: {fwd_time}"
                )
                ...
                if (idx + 1) % self._gradient_accumulation_steps == 0:
                    ...
                    optim_time = optim_end - bwd_end
                    log.info(
                        f"[rank {self.rank} step {self.global_step}] optim time: {optim_time}"
                    )
